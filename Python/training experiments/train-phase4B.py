...
# Output Dirs (Phase 4B - Weighted Loss)
RESULTS_DIR = "Results_Phase4B_Weighted_Entailment"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32 
EPOCHS = 3            # Keeping it fast
LEARNING_RATE = 5e-4 
MAX_LEN = 64
TEST_SPLIT = 0.20
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 4B: Learning Curve (Weighted Loss)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4B_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 4B Confusion Matrix (Weighted)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4B_confusion_matrix.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 4B (Weighted Loss) Results:\n"
        f"---------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase4B_metrics.txt"), "w") as f:
        f.write(stats_text)

    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase4B_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase4B_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Starting Phase 4B (Weighted Loss) on {device}...")
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase4B_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 4B Complete! Results in: {RESULTS_DIR}")
