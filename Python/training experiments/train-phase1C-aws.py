...
RESULTS_DIR = "Results_Phase1C_OneHot"
...
class ThesisOneHotNet(nn.Module):
    def __init__(self):
        ...
        # 3. Fusion 
        # 512 (Vis) + 768 (Text) + 9 (One-Hot Vector) = 1289
    ...
    def forward(self, images, input_ids, attention_mask, one_hots):
        # Vision
        ...
        combined = torch.cat((vis_feat, txt_feat, one_hots), dim=1)
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 1C: Learning Curve (One-Hot)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.title('Phase 1C Residuals: One-Hot Errors')
    ...
    print(f"📋 Full predictions saved to: Phase1C_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Using Mixed Precision for Speed on AWS
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in loop:
            ...
            cats = batch['category_onehot'].to(device) 
    ...
    # Eval Train Loader
    train_eval_loader = DataLoader(ThesisDataset(train_df, tokenizer, transform, 'train', LMDB_PATH), batch_size=BATCH_SIZE, shuffle=False, num_workers=8, pin_memory=True)
