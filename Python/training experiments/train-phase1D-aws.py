...
# Output Dirs (Phase 1D)
RESULTS_DIR = "Results_Phase1D_FashionOnly"
...
# --- 1. DATASET CLASS (Standard Image + Text) ---
...
# --- 2. THE MODEL (No Category  - Pure Vision+Text) ---
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 1D: Learning Curve (Fashion Only)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase1D_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 1D Residuals: Fashion Specialist Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase1D_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 1D (Fashion Only) Results:\n"
        f"--------------------------------\n"
        f"R2 Score (Train): {r2_train:.4f}\n"
        f"R2 Score (Test):  {r2_test:.4f}\n"
        f"MAE (Test):       {mae_test:.4f}\n"
        f"Samples Used:     {len(full_df)}\n"
    )
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase1D_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase1D_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase1D_predictions_full.csv")

# --- 4. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 1D (Fashion Specialist) on {device}...")
    
    if not os.path.exists(MANIFEST_FILE):
        print(f"❌ Error: Manifest not found.")
        return

    # --- FILTERING FOR FASHION ---
    ...
    print(f"    Filtered Data: {len(df):,} rows (dropped {initial_len - len(df):,} non-fashion items)")
    
    if len(df) == 0:
        print("❌ Error: No Fashion items found! Check category names.")
        return
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase1D_model_epoch_{epoch+1}.pth"))
    ...
    train_eval_loader = DataLoader(ThesisDataset(train_df, tokenizer, transform, 'train'), batch_size=BATCH_SIZE, shuffle=False, num_workers=4)
    ...
    print(f"\n🎉 Phase 1D Complete! Results in: {RESULTS_DIR}")
