...
# Output Dirs (Phase 1E)
RESULTS_DIR = "Results_Phase1E_ComplexFusion"
...
# --- 3. THE MODEL (Updated Fusion: Wide -> Skinny -> Wide) ---
class ThesisCategoryNet(nn.Module):
    def __init__(self):
        ...
        # Combined Input: 512 + 768 + 32 = 1312
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 1E: Learning Curve (Wide-Skinny-Wide)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase1E_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 1E Residuals')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase1E_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 1E (Complex Fusion) Results:\n"
        f"----------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase1E_metrics.txt"), "w") as f:
        f.write(stats_text)

    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase1E_predictions_full.csv"), index=False)

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 1E (Complex Fusion) on {device}...")
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase1E_model_epoch_{epoch+1}.pth"))
    ...
    scan(val_loader)
    scan(DataLoader(ThesisDataset(train_df, tokenizer, transform, 'train'), batch_size=BATCH_SIZE, shuffle=False))

    final_df = pd.DataFrame(all_results)
    analyze_and_save(history, final_df)
    print(f"\n🎉 Phase 1E Complete! Results in: {RESULTS_DIR}")
