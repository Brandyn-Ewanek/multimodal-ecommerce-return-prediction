...
# Output Dirs (Phase 2B - Cosine Similarity)
RESULTS_DIR = "Results_Phase2B_CosineSim"
...
# --- 3. THE MODEL (Phase 1B Base + Cosine Similarity) ---
class ThesisCosineNet(nn.Module):
    def __init__(self):
        super(ThesisCosineNet, self).__init__()
        ...
        # 4. ALIGNMENT LAYER (New for Phase 2B)
        # We must shrink Text (768) -> 512 to calculate Cosine with Image (512)
        self.text_align = nn.Linear(768, 512)
        
        # Fusion Input Calculation:
        # Image (512) + Text (768) + Category (32) + CosineScore (1) = 1313
        self.fusion = nn.Sequential(
            nn.Linear(1313, 512),  # Note the extra +1 input
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 1)
    ...
    def forward(self, images, input_ids, attention_mask, categories):
        ...
        # B. Calculate Cosine Similarity (The "Cheat Sheet")
        # 1. Project text to match image dimension
        txt_aligned = self.text_align(txt_feat) # [Batch, 512]
        
        # 2. Normalize both vectors (Cosine = Dot product of normalized vectors)
        vis_norm = F.normalize(vis_feat, p=2, dim=1)
        txt_norm = F.normalize(txt_aligned, p=2, dim=1)
        
        # 3. Calculate Dot Product (Result is Cosine Similarity)
        # We unsqueeze(1) to make it [Batch, 1] so we can concatenate later
        cosine_sim = torch.sum(vis_norm * txt_norm, dim=1).unsqueeze(1) 
        
        # C. Fusion
        # We feed the Original Text (768), not the aligned one, to preserve Phase 1B info
        combined = torch.cat((vis_feat, txt_feat, cat_feat, cosine_sim), dim=1)
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 2B: Learning Curve (Cosine Sim)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2B_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 2B Residuals: Cosine Sim Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2B_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 2B (Cosine Similarity) Results:\n"
        f"-------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase2B_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase2B_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase2B_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 2B (Cosine Similarity) on {device}...")
    ...
    model = ThesisCosineNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase2B_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 2B Complete! Results in: {RESULTS_DIR}")
