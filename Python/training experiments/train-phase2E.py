...
# Output Dirs (Phase 2E - The Forensic Model)
RESULTS_DIR = "Results_Phase2E_Forensic_Full"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32
EPOCHS = 6             # INCREASED TO 6
LEARNING_RATE = 1e-5   # SLOWER LR
MAX_LEN = 64
TEST_SPLIT = 0.20
...
# --- 3. THE FORENSIC MODEL (All Features + Gating) ---
class ThesisForensicNet(nn.Module):
    def __init__(self):
        super(ThesisForensicNet, self).__init__()
        ...
        # 4. ALIGNMENT LAYER (Text -> Image Dimension)
        self.text_align = nn.Linear(768, 512)
        
        # INPUT CALCULATION:
        # Core: Vis(512) + Txt(768) + Cat(32) = 1312
        # Geo Features: Cosine(1) + Euclidean(1) + Rejection(1) = 3
        # Total Input: 1315
        input_dim = 1315
        hidden_dim = 512
    ...
    def forward(self, images, input_ids, attention_mask, categories):
        ...
        # B. Calculate Geometric Evidence
        # 1. Align Text
        ...
        # 3. Cosine Similarity (Dot Product of Norms)
        ...
        # 5. Vector Rejection Magnitude
        # Projection of Text onto Image
        scalar_proj = torch.sum(txt_aligned * vis_norm, dim=1, keepdim=True)
        vector_proj = scalar_proj * vis_norm
        # Rejection = Text - Projection
        rejection_vect = txt_aligned - vector_proj
        rejection_mag = torch.norm(rejection_vect, p=2, dim=1, keepdim=True)
        
        # C. Combine Everything
        combined = torch.cat((vis_feat, txt_feat, cat_feat, cosine_sim, euclidean_dist, rejection_mag), dim=1)
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 2E: Learning Curve (Full Forensic Model)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2E_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 2E Residuals: Forensic Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2E_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 2E (Full Forensic Model) Results:\n"
        f"---------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase2E_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase2E_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase2E_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 2E (Full Forensic Model) on {device}...")
    ...
    model = ThesisForensicNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase2E_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 2E Complete! Results in: {RESULTS_DIR}")
