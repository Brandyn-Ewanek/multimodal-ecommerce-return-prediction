...
# Output Dirs (Phase 3F - SigCLIP Regularized)
RESULTS_DIR = "Results_Phase3F_SigCLIP_Regularized"
...
# --- 3. THE MODEL (SigCLIP + Regularization) ---
class ThesisSiglipRegNet(nn.Module):
    def __init__(self):
        super(ThesisSiglipRegNet, self).__init__()
        ...
        # 1. Bottleneck: Squeezing massive 1568 inputs into small 128 vector
        input_dim = vis_dim + txt_dim + cat_dim 
        bottleneck_dim = 128  # <--- Drastically reduced from 768
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3F: Learning Curve (SigCLIP Regularized)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3F_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3F Residuals: SigCLIP Reg Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3F_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3F (SigCLIP Regularized) Results:\n"
        f"-------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3F_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3F_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3F_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3F (SigCLIP Regularized) on {device}...")
    ...
    model = ThesisSiglipRegNet().to(device)
    
    # --- ADDED WEIGHT DECAY ---
    # weight_decay=0.01 penalizes large weights, preventing overfitting
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3F_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 3F Complete! Results in: {RESULTS_DIR}")
