...
# Output Dirs (Phase 4D - Weighted Loss 1.35x)
RESULTS_DIR = "Results_Phase4D_Weighted_Entailment_1.35x"
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 4D: Learning Curve (Weighted Loss 1.35x)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4D_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 4D Confusion Matrix (Weighted 1.35x)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4D_confusion_matrix.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 4D (Weighted Loss 1.35x) Results:\n"
        f"---------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase4D_metrics.txt"), "w") as f:
        f.write(stats_text)

    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase4D_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase4D_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Starting Phase 4D (Weighted Loss 1.35x) on {device}...")
    ...
    # Safe (0) = 1.0
    # Risky (1) = 1.35 (The Gentle Push)
    class_weights = torch.tensor([1.0, 1.35]).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase4D_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 4D Complete! Results in: {RESULTS_DIR}")
