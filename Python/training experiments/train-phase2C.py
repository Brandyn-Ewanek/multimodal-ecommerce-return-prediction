...
# Output Dirs (Phase 2C - Vector Rejection)
RESULTS_DIR = "Results_Phase2C_RejectionMag"
...
# --- 3. THE MODEL (Phase 1B Base + Vector Rejection) ---
class ThesisRejectionNet(nn.Module):
    def __init__(self):
        super(ThesisRejectionNet, self).__init__()
        
        # 1. Vision Leg (ResNet18)
        self.visual = models.resnet18(pretrained=True)
        self.visual.fc = nn.Identity() # Output: 512
        
        # 2. Text Leg (DistilBERT)
        ...
        # 4. ALIGNMENT LAYER (Required for vector math)
        # We must shrink Text (768) -> 512 to interact with Image (512)
        self.text_align = nn.Linear(768, 512)
        
        # Fusion Input Calculation:
        # Image (512) + Text (768) + Category (32) + RejectionMag (1) = 1313
    ...
    def forward(self, images, input_ids, attention_mask, categories):
        ...
        # B. Calculate Vector Rejection (The "Hallucination" Score)
        # 1. Align Text to Image Space
        txt_aligned = self.text_align(txt_feat) # [Batch, 512]
        
        # 2. Normalize Image to create a "Direction" (Unit Vector)
        # We project Text onto the Image Direction
        vis_norm = F.normalize(vis_feat, p=2, dim=1)
        
        # 3. Calculate Scalar Projection (Dot Product)
        scalar_proj = torch.sum(txt_aligned * vis_norm, dim=1, keepdim=True)
        
        # 4. Calculate Vector Projection
        vector_proj = scalar_proj * vis_norm
        
        # 5. Calculate Rejection Vector (Text - Projection)
        rejection_vector = txt_aligned - vector_proj
        
        # 6. Rejection Magnitude (Euclidean Norm of the rejection vector)
        rejection_mag = torch.norm(rejection_vector, p=2, dim=1, keepdim=True)
        
        # C. Fusion
        combined = torch.cat((vis_feat, txt_feat, cat_feat, rejection_mag), dim=1)
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 2C: Learning Curve (Rejection Mag)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2C_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 2C Residuals: Rejection Magnitude Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase2C_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 2C (Vector Rejection) Results:\n"
        f"------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase2C_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase2C_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase2C_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 2C (Vector Rejection) on {device}...")
    ...
    model = ThesisRejectionNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase2C_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 2C Complete! Results in: {RESULTS_DIR}")
