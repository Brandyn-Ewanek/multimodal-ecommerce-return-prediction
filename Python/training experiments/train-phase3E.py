...
# Output Dirs (Phase 3E - SigCLIP)
RESULTS_DIR = "Results_Phase3E_SigCLIP"
...
# --- 3. THE MODEL (SigCLIP) ---
class ThesisSiglipNet(nn.Module):
    def __init__(self):
        super(ThesisSiglipNet, self).__init__()
        ...
        # Feature Dimensions (SigCLIP Base is 768)
        vis_dim = 768
        txt_dim = 768
        cat_dim = 32
    ...
    def forward(self, pixel_values, input_ids, attention_mask, categories):
        ...
        img_embeds = outputs.image_embeds  # [Batch, 768]
        txt_embeds = outputs.text_embeds   # [Batch, 768]
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3E: Learning Curve (SigCLIP)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3E_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3E Residuals: SigCLIP Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3E_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3E (SigCLIP Fine-Tuned) Results:\n"
        f"-------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3E_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3E_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3E_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3E (SigCLIP) on {device}...")
    ...
    try:
        ...
        print(f"   ⚠️ Error loading SiglipProcessor: {e}")
    ...
    model = ThesisSiglipNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in loop:
            ...
            # --- FIXED: Use the key we safely created in __getitem__ ---
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3E_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 3E Complete! Results in: {RESULTS_DIR}")
