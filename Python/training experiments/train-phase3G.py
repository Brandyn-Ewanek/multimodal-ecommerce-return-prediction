...
# Output Dirs (Phase 3G - SigCLIP LoRA)
RESULTS_DIR = "Results_Phase3G_SigCLIP_LoRA"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32 
EPOCHS = 3
LEARNING_RATE = 5e-4  # <--- HIGHER LR is safe for LoRA (standard practice)
MAX_LEN = 64
TEST_SPLIT = 0.20
...
# --- 2. DATASET CLASS ---
class ThesisSiglipDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 4. SAFELY HANDLE ATTENTION MASK (The SigLIP Fix)
...
class ThesisSiglipLoRANet(nn.Module):
    def __init__(self):
        super(ThesisSiglipLoRANet, self).__init__()
        ...
        # Config: Rank 16 (standard), Alpha 32, Target Attention Projections
        peft_config = LoraConfig(
            r=16,
            lora_alpha=32,
            target_modules=["q_proj", "v_proj"], # Target Query/Value in Attention
            lora_dropout=0.1,
            bias="none"
        )
        ...
        self.backbone.print_trainable_parameters() # This will print how many weights we are training
    ...
    def forward(self, pixel_values, input_ids, attention_mask, categories):
        # Pass through LoRA-wrapped backbone
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3G: Learning Curve (SigCLIP LoRA)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3G_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3G Residuals: LoRA Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3G_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3G (SigCLIP LoRA) Results:\n"
        f"-------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3G_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3G_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3G_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3G (SigCLIP LoRA) on {device}...")
    ...
    print("   Initializing SiglipProcessor...")
    try:
    ...
    train_loader = DataLoader(ThesisSiglipDataset(train_df, processor, 'train', LMDB_PATH), 
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    val_loader = DataLoader(ThesisSiglipDataset(val_df, processor, 'test', LMDB_PATH), 
                            batch_size=BATCH_SIZE, shuffle=False, num_workers=8, pin_memory=True)
    
    model = ThesisSiglipLoRANet().to(device)
    
    # Optimize everything (Backbone params are now mostly frozen by PEFT, but Fusion Head is open)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3G_model_epoch_{epoch+1}.pth"))
    ...
    scan(DataLoader(ThesisSiglipDataset(train_df, processor, 'train', LMDB_PATH), batch_size=BATCH_SIZE, shuffle=False, num_workers=8), "Train")

    final_df = pd.DataFrame(all_results)
    analyze_and_save(history, final_df)
    print(f"\n🎉 Phase 3G Complete! Results in: {RESULTS_DIR}")
