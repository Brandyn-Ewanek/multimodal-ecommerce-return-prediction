...
# Output Dirs (Phase 4E - Weighted Loss 1.15x)
RESULTS_DIR = "Results_Phase4E_Weighted_Entailment_1.15x"
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 4E: Learning Curve (Weighted Loss 1.15x)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4E_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 4E Confusion Matrix (Weighted 1.15x)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4E_confusion_matrix.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 4E (Weighted Loss 1.15x) Results:\n"
        f"---------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase4E_metrics.txt"), "w") as f:
        f.write(stats_text)

    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase4E_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase4E_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Starting Phase 4E (Weighted Loss 1.15x) on {device}...")
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase4E_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 4E Complete! Results in: {RESULTS_DIR}")
