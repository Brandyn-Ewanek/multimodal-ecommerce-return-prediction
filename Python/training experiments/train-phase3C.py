...
# Output Dirs (Phase 3C - Forensic CLIP)
RESULTS_DIR = "Results_Phase3C_CLIP_Forensic"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32
EPOCHS = 3
LEARNING_RATE = 5e-6    # Keep Low LR from 3B
MAX_LEN = 77
TEST_SPLIT = 0.20
...
# --- 3. THE MODEL (Forensic CLIP) ---
class ThesisForensicCLIPNet(nn.Module):
    def __init__(self):
        super(ThesisForensicCLIPNet, self).__init__()
        ...
        # Fusion Head Inputs:
        # Image(512) + Text(512) + Cat(32) + Cosine(1) + Euclidean(1) = 1058
    ...
    def forward(self, pixel_values, input_ids, attention_mask, categories):
        ...
        img_embeds = outputs.image_embeds # [Batch, 512]
        txt_embeds = outputs.text_embeds  # [Batch, 512]
        
        # 2. Geometric Metrics (The "Forensic" Check)
        # Normalize for Cosine
        img_norm = F.normalize(img_embeds, p=2, dim=1)
        txt_norm = F.normalize(txt_embeds, p=2, dim=1)
        
        # A. Cosine Similarity (Dot Product of Norms)
        cosine_sim = torch.sum(img_norm * txt_norm, dim=1).unsqueeze(1)
        
        # B. Euclidean Distance (Raw Distance)
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3C: Learning Curve (Forensic CLIP)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3C_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3C Residuals: Forensic CLIP Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3C_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3C (Forensic CLIP) Results:\n"
        f"---------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3C_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3C_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3C_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3C (Forensic CLIP) on {device}...")
    ...
    model = ThesisForensicCLIPNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3C_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 3C Complete! Results in: {RESULTS_DIR}")
