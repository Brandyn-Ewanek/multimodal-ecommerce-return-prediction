...
# Output Dirs (Phase 3H - SigCLIP LoRA Long)
RESULTS_DIR = "Results_Phase3H_SigCLIP_LoRA_Long"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32 
EPOCHS = 9            # INCREASED TO 9
LEARNING_RATE = 5e-4  # High LR  for LoRA
MAX_LEN = 64
TEST_SPLIT = 0.20
...
class ThesisSiglipLoRALongNet(nn.Module):
    def __init__(self):
        super(ThesisSiglipLoRALongNet, self).__init__()
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3H: Learning Curve (SigCLIP LoRA Long)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3H_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3H Residuals: LoRA Long Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3H_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3H (SigCLIP LoRA Long 9 Epochs) Results:\n"
        f"---------------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3H_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3H_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3H_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3H (SigCLIP LoRA Long) on {device}...")
    ...
    model = ThesisSiglipLoRALongNet().to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3H_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 3H Complete! Results in: {RESULTS_DIR}")
