...
# Output Dirs (Phase 3B - Fine-Tuned CLIP)
RESULTS_DIR = "Results_Phase3B_CLIP_FineTune"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32
EPOCHS = 3
LEARNING_RATE = 5e-6    # LOWERED for Fine-Tuning (Safety)
MAX_LEN = 77
TEST_SPLIT = 0.20
...
class ThesisCLIPNet(nn.Module):
    def __init__(self):
        ...
        # UNFREEZE CLIP 
        # We allow gradients to flow back into the vision/text encoders
    ...
    def forward(self, pixel_values, input_ids, attention_mask, categories):
        # 1. Extract Features (Now Trainable)
        outputs = self.clip(
            input_ids=input_ids, 
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 3B: Learning Curve (CLIP Fine-Tuned)')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3B_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 3B Residuals: CLIP Fine-Tuned Errors')
    plt.xlabel('Actual Return Risk')
    plt.ylabel('Error (Predicted - Actual)')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase3B_residuals.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 3B (CLIP Fine-Tuned) Results:\n"
        f"-----------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase3B_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase3B_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase3B_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    scaler = torch.amp.GradScaler('cuda') if torch.cuda.is_available() else None
    print(f"🚀 Starting Phase 3B (CLIP Fine-Tuned) on {device}...")
    ...
    # Optimizer (Now optimizes everything)
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase3B_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 3B Complete! Results in: {RESULTS_DIR}")
