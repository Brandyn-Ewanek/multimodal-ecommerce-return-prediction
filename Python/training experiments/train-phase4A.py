...
# Output Dirs (Phase 4A - Visual Entailment)
RESULTS_DIR = "Results_Phase4A_VisualEntailment"
os.makedirs(RESULTS_DIR, exist_ok=True)

BATCH_SIZE = 32 
EPOCHS = 9            # Keeping 9 epochs (like Phase 3H)
LEARNING_RATE = 5e-4  # LoRA LR
MAX_LEN = 64
TEST_SPLIT = 0.20

# --- THRESHOLDS (Dropping the Middle) ---
SAFE_LIMIT = 0.4
RISK_LIMIT = 0.6
# Logic: 
#   Score <= 0.4 -> Class 0 (Safe)
#   Score >= 0.6 -> Class 1 (Risky)
#   0.4 < Score < 0.6 -> DROPPED
...
# --- 3. THE MODEL (SigCLIP LoRA Classifier) ---
class ThesisSiglipClassifier(nn.Module):
    def __init__(self):
        ...
        # --- 🛡️ APPLY LoRA (Same as Phase 3H) 🛡️ ---
...
def analyze_and_save(history, full_df):
    ...
    plt.title('Phase 4A: Learning Curve (Visual Entailment)')
    plt.xlabel('Epochs')
    plt.ylabel('Cross Entropy Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4A_loss_curve.png"))
    plt.close()
    ...
    plt.title('Phase 4A Confusion Matrix')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.savefig(os.path.join(RESULTS_DIR, "Phase4A_confusion_matrix.png"))
    plt.close()
    ...
    stats_text = (
        f"Phase 4A (Visual Entailment - No Middle) Results:\n"
        f"-------------------------------------------------\n"
    ...
    print("\n" + stats_text)
    with open(os.path.join(RESULTS_DIR, "Phase4A_metrics.txt"), "w") as f:
        f.write(stats_text)

    # 4. Save CSV
    full_df.to_csv(os.path.join(RESULTS_DIR, "Phase4A_predictions_full.csv"), index=False)
    print(f"📋 Full predictions saved to: Phase4A_predictions_full.csv")

# --- 5. MAIN TRAINING LOOP ---
def run_training():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 Starting Phase 4A (Task Reformulation) on {device}...")
    ...
    # 1. Standard Split (Keep Random State 42!)
    train_df_full, val_df_full = train_test_split(df, test_size=TEST_SPLIT, random_state=42)
    print(f"   📚 Full Split -> Train: {len(train_df_full):,} | Test: {len(val_df_full):,}")
    
    # 2. FILTER: DROP THE MIDDLE (0.4 to 0.6)
    def filter_middle(d):
    ...
    print(f"   ✂️  Filtered (Dropped Middle) -> Train: {len(train_df):,} | Test: {len(val_df):,}")
    
    # Check Balance
    risk_count = (train_df['return_likelihood'] >= RISK_LIMIT).sum()
    safe_count = len(train_df) - risk_count
    print(f"   ⚖️  Class Balance (Train): Safe={safe_count}, Risky={risk_count}")
    ...
    for epoch in range(EPOCHS):
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"Phase4A_model_epoch_{epoch+1}.pth"))
    ...
    print(f"\n🎉 Phase 4A Complete! Results in: {RESULTS_DIR}")
